...
def index(movie=None,querry=None,results=None):
    if request.method == 'POST':
        ...
        for row in c.execute("""SELECT neigbhoors FROM konate-malick-recommend-movies.Content WHERE original_title='%s'""" % movie):
        ...
        for neighboor in myneighbs:
            for row2 in c.execute("""SELECT moviesynthesis FROM konate-malick-recommend-movies.Content WHERE movie_index='%s'""" % neighboor):
