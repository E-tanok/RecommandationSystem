...
def index(movie=None,querry=None,results=None):
    if request.method == 'POST':
        ...
        for row in c.execute("""SELECT neigbhoors FROM postgresql-lively-43539.Content WHERE original_title='%s'""" % movie):
        ...
        for neighboor in myneighbs:
            for row2 in c.execute("""SELECT moviesynthesis FROM postgresql-lively-43539.Content WHERE movie_index='%s'""" % neighboor):
